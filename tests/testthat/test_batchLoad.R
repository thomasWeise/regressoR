...
.test <- function(cores) {
  ...
  results <- regressoR::regressoR.batchLoad(source=file.path(data[1], "models"),
                                            cores=cores,
                                            logging=FALSE);

  expect_length(results, 3L);

  expect_identical(results[[1]]@names, c("a"));
  expect_length(results[[1]]@results, 4L);
  expect_equal(results[[1]]@results[[1]], output[[1]]);
  expect_equal(results[[1]]@results[[2]], output[[2]]);
  expect_equal(results[[1]]@results[[3]], output[[3]]);
  expect_equal(results[[1]]@results[[4]], output[[4]]);

  expect_identical(results[[2]]@names, c("b"));
  expect_length(results[[2]]@results, 4L);
  expect_equal(results[[2]]@results[[1]], output[[5]]);
  expect_equal(results[[2]]@results[[2]], output[[6]]);
  expect_equal(results[[2]]@results[[3]], output[[7]]);
  expect_equal(results[[2]]@results[[4]], output[[8]]);

  expect_identical(results[[3]]@names, c("c"));
  expect_length(results[[3]]@results, 3L);
  expect_equal(results[[3]]@results[[1]], output[[9]]);
  expect_equal(results[[3]]@results[[2]], output[[10]]);
  expect_equal(results[[3]]@results[[3]], output[[11]]);

  unlink(data[1], recursive = TRUE);
}


test_that("Test regressoR.batchLoad I", {
  .test(cores=1L);
})

test_that("Test regressoR.batchLoad II", {
  .test(cores=2L);
})

test_that("Test regressoR.batchLoad III", {
  if(slow.tests) { .test(cores=3L); }
})

test_that("Test regressoR.batchLoad IIII", {
